...
Only slightly modified from: https://github.com/openvla/openvla/blob/main/experiments/robot/libero/regenerate_libero_dataset.py
...
def regenerate_libero_dataset(cfg):
    print(f"Regenerating {cfg.libero_task_suite} dataset!")

    # Create target directory
    if os.path.isdir(cfg.libero_target_dir):
        user_input = input(
            f"Target directory already exists at path: {cfg.libero_target_dir}\nEnter 'y' to overwrite the directory, or anything else to exit: "
        )
        if user_input != "y":
            exit()
    ...
    metainfo_json_dict = {}
    metainfo_json_out_path = f"{cfg.libero_task_suite}_metainfo.json"
    ...
    task_suite = benchmark_dict[cfg.libero_task_suite]()
...
@hydra.main(config_path="../../config/dataset", config_name="regenerate_libero")
def main(cfg: OmegaConf):
    regenerate_libero_dataset(cfg)
