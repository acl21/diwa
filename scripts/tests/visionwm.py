...
def main(cfg):
    wm_path = f"{os.environ['DIWA_LOG_DIR']}/wm/ckpts/vision.ckpt"
    stats_path = f"{os.environ['DIWA_DATA_DIR']}/statistics_minmax.yaml"
    device = "cuda:0"
    skills = [
        "open_drawer",
        "close_drawer",
    ...
    for skill_name in tqdm(skills, desc="Skills"):
        ...
        robot_obs, scene_obs = np.split(obs[0], [18])
        ...
        for i in range(len(obs_feat)):
            pred_next_latent = wmw.wm_step(
                wm_features,
                torch.from_numpy(actions[i]).to(device).unsqueeze(0).float(),
            )
