...
from diwa.model.diffusion.sampling import cosine_beta_schedule, extract, make_timesteps
...
class DiffusionModel(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        logging.info(f"Number of network parameters: {sum(p.numel() for p in self.parameters())}")
        ...
        if use_ddim:
            ...
            self.ddim_alphas_prev = torch.cat(
                [torch.tensor([1.0]).to(torch.float32).to(self.device), self.alphas_cumprod[self.ddim_t[:-1]]]
            )
