...
from diwa.model.common.mlp import MLP
...
class EtaFixed(torch.nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.eta_logit.data = torch.atanh(torch.tensor([2 * (base_eta - min_eta) / (max_eta - min_eta) - 1]))
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.eta_logit = torch.nn.Parameter(
            torch.ones(action_dim) * torch.atanh(torch.tensor([2 * (base_eta - min_eta) / (max_eta - min_eta) - 1]))
        )
        self.min = min_eta
        self.max = max_eta
...
class EtaState(torch.nn.Module):
    ...
    def __call__(self, cond):
        if "rgb" in cond:
            raise NotImplementedError("State-based eta not implemented for image-based training!")
        # flatten history
        B = len(cond["state"])
...
class EtaStateAction(torch.nn.Module):
    ...
    def __call__(self, cond):
        if "rgb" in cond:
            raise NotImplementedError("State-action-based eta not implemented for image-based training!")
        # flatten history
        B = len(cond["state"])
        state = cond["state"].view(B, -1)
