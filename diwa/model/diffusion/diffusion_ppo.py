...
from diwa.model.diffusion.diffusion_vpg import VPGDiffusion
    ...
    def loss(
        ...
        if use_bc_loss:
            ...
            samples = self.forward(
                cond=obs,  # B x horizon=1 x obs_dim
                deterministic=False,
                return_chain=True,
        ...
        # denoising discount
        discount = torch.tensor([self.gamma_denoising**i for i in reversed(range(self.ft_denoising_steps))]).to(
            self.device
        )
        ...
        t = torch.arange(self.ft_denoising_steps).float().to(self.device) / (self.ft_denoising_steps - 1)  # 0 to 1
        if self.ft_denoising_steps > 1:
            clip_ploss_coef = self.clip_ploss_coef_base + (self.clip_ploss_coef - self.clip_ploss_coef_base) * (
        ...
        clip_ploss_coef = clip_ploss_coef.repeat(len(advantages) // self.ft_denoising_steps)
        ...
        pg_loss2 = -advantages * torch.clamp(ratio, 1 - clip_ploss_coef, 1 + clip_ploss_coef)
