...
from diwa.model.diffusion.diffusion import DiffusionModel
from diwa.model.diffusion.sampling import extract
...
class DiffusionEval(DiffusionModel):
    def __init__(
        ...
        **kwargs,
        ...
        try:
            base_weights = {
                key.split("actor.")[1]: checkpoint["model"][key] for key in checkpoint["model"] if "actor." in key
            }
            use_ft = True
            self.actor.load_state_dict(base_weights, strict=True)
        except Exception:
            assert ft_denoising_steps == 0, "If no base policy weights are found, ft_denoising_steps must be 0"
            base_weights = {
                key.split("network.")[1]: checkpoint["model"][key] for key in checkpoint["model"] if "network." in key
            }
            use_ft = False
        ...
        if use_ft:
            ...
            ft_weights = {
                key.split("actor_ft.")[1]: checkpoint["model"][key] for key in checkpoint["model"] if "actor_ft." in key
            }
    ...
    def p_mean_var(
        ...
        if self.use_ddim:
            ...
            sigma = (etas * ((1 - alpha_prev) / (1 - alpha) * (1 - alpha / alpha_prev)) ** 0.5).clamp_(min=1e-10)
            ...
            """
            mu = extract(self.ddpm_mu_coef1, t, x.shape) * x_recon + extract(self.ddpm_mu_coef2, t, x.shape) * x
