...
from diwa.model.common.mlp import MLP, ResidualMLP
from diwa.model.common.modules import RandomShiftsAug, SpatialEmb
...
class Gaussian_VisionMLP(nn.Module):
    ...
    def forward(self, cond):
        ...
        out_mean = torch.tanh(out_mean).view(B, self.horizon_steps * self.action_dim)  # tanh squashing in [-1, 1]
        ...
        if self.learn_fixed_std:
            ...
            out_logvar = self.mlp_logvar(x_encoded).view(B, self.horizon_steps * self.action_dim)
...
class Gaussian_MLP(nn.Module):
    ...
    def forward(self, cond):
        ...
        if self.learn_fixed_std:
            ...
            out_logvar = self.mlp_logvar(state).view(B, self.horizon_steps * self.action_dim)
