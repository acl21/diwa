...
from diwa.model.diffusion.modules import SinusoidalPosEmb
...
class Gaussian_Transformer(nn.Module):
    def __init__(
        ...
        self.logvar_min = torch.nn.Parameter(torch.log(torch.tensor(std_min**2)), requires_grad=False)
        self.logvar_max = torch.nn.Parameter(torch.log(torch.tensor(std_max**2)), requires_grad=False)
...
class GMM_Transformer(nn.Module):
    def __init__(
        ...
        if fixed_std is None:
            ...
            logger.info("Using learned std")
        elif learn_fixed_std:  # initialize to fixed_std, separate for each action and mode, but same along horizon
            self.logvar = torch.nn.Parameter(
                torch.log(torch.tensor([fixed_std**2 for _ in range(num_modes * action_dim)])),
                requires_grad=True,
            )
            logger.info(f"Using fixed std {fixed_std} with learning")
        else:
    ...
    def forward(self, cond):
        ...
        # input to transformer
        state = state.unsqueeze(1)  # (B,1,cond_dim)
        out, out_prehead = self.transformer(state)  # (B,horizon,output_dim), (B,horizon,emb_dim)

        # use the first half of the output as mean
        out_mean = torch.tanh(out[:, :, : self.num_modes * self.action_dim])
        out_mean = out_mean.reshape(B, self.horizon_steps, self.num_modes, self.action_dim)
        out_mean = out_mean.permute(0, 2, 1, 3)  # flip horizons and modes
        out_mean = out_mean.reshape(B, self.num_modes, self.horizon_steps * self.action_dim)
        ...
        if self.learn_fixed_std:
            ...
            out_logvar = out_logvar.reshape(B, self.horizon_steps, self.num_modes, self.action_dim)
            out_logvar = out_logvar.permute(0, 2, 1, 3)  # flip horizons and modes
            out_logvar = out_logvar.reshape(B, self.num_modes, self.horizon_steps * self.action_dim)
            out_logvar = torch.clamp(out_logvar, self.logvar_min, self.logvar_max)
            out_scale = torch.exp(0.5 * out_logvar)
...
class Transformer(nn.Module):
    def __init__(
        ...
        if causal_attn:
            ...
            t, s = torch.meshgrid(torch.arange(horizon), torch.arange(T_cond), indexing="ij")
    ...
    def forward(
        ...
        **kwargs,
        ...
        # decoder
        position_embeddings = self.pos_emb[:, : self.horizon, :]  # each position maps to a (learnable) vector
        position_embeddings = position_embeddings.expand(cond.shape[0], self.horizon, -1)  # repeat for batch dimension
