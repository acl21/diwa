# Basicaly a copy of the original file from stable-baselines3
# https://github.com/DLR-RM/stable-baselines3/blob/master/stable_baselines3/common/vec_env/subproc_vec_env.py
# Supports termination and truncation signals from the environment
...
import numpy as np
from stable_baselines3.common.vec_env.base_vec_env import CloudpickleWrapper, VecEnv, VecEnvIndices, VecEnvObs
from stable_baselines3.common.vec_env.patch_gym import _patch_env

CustomVecEnvStepReturn = Tuple[VecEnvObs, np.ndarray, np.ndarray, np.ndarray, List[Dict]]
...
class CustomSubprocVecEnv(VecEnv):
    ...
    def step_wait(self) -> CustomVecEnvStepReturn:
        results = [remote.recv() for remote in self.remotes]
        self.waiting = False
        obs, rews, terminated, truncated, infos, self.reset_infos = zip(*results)  # type: ignore[assignment]
        return _flatten_obs(obs, self.observation_space), np.stack(rews), np.stack(terminated), np.stack(truncated), infos  # type: ignore[return-value]
