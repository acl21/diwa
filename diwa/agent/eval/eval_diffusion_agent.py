"""
Evaluate pre-trained/DiWA-fine-tuned diffusion policy.
...
class EvalDiffusionAgent(EvalAgent):
    ...
    def run(self):
        ...
        time = timer()
        log.info(
            f"eval: num episode {num_episode_finished:4d} | success rate {success_rate:8.4f} | avg episode reward {avg_episode_reward:8.4f} | avg best reward {avg_best_reward:8.4f} | avg episode length {avg_episode_length:8.4f}"
        )
        np.savez(
            self.result_path,
            ...
            episode_length=avg_episode_length * self.act_steps,
            time=time,
        )
