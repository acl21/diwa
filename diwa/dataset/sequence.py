"""
Unmodified from https://github.com/irom-princeton/dppo
"""
...
class StitchedSequenceDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        traj_lengths = dataset["traj_lengths"].astype(np.int64)  # 1-D array
        ...
        if self.use_img:
            self.images = torch.from_numpy(dataset["images"][:total_num_steps]).to(device)  # (total_num_steps, C, H, W)
    ...
    def __getitem__(self, idx):
        ...
        if self.use_img:
            ...
            images = torch.stack([images[max(num_before_start - t, 0)] for t in reversed(range(self.img_cond_steps))])
